...
async function generateUniqueNumber2(timeNum, transId, comment, winRate = 30) {
    ...
    let M_ArrayNum = [2, 4, 6, 8]; // chẳn , chẳn 1
    let R_ArrayNum = [3, 5, 7, 9] // lẻ , lẻ 1
    let X_ArrayNum = [1, 2, 3, 4] // xỉu, xỉu 1
    let T_ArrayNum = [5, 6, 7, 8] // tài, tài 1

    let M_Array = GameList.filter(item => {
        return M_ArrayNum.every(number => item.numberTLS.includes(number));
      }).map(item => item.content);
    let R_Array = GameList.filter(item => {
        return R_ArrayNum.every(number => item.numberTLS.includes(number));
      }).map(item => item.content);
    ...
    if ((M_Array.includes(comment) && soChan == 1) ||
        (R_Array.includes(comment) && soChan == 0)
    ) {
    ...
    else if ((M_Array.includes(comment) && soChan == 0)
        || (R_Array.includes(comment) && soChan == 1)
    ) {
        let lastRD = getRndInteger(0, 4);
        if (rd < winRate) {
            last = (2 * lastRD);
