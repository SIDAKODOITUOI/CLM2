...
function loadHistory(err, response) {
    ...
    response.data.map(data=>{
        $("#tableHistory").append(`<tr><td>${data.phone}</td><td>${data.transId}</td><td>${data.random}</td><td>${data.transNew}</td><td>${Intl.NumberFormat("en-US").format(data.amount)} đ</td><td>${data.content}</td>${"win" == data.status ? '<td><span class="badge badge-success">Thắng</span></td>' : '<td><span class="badge badge-dark">Thua</span></td>'}</tr>`)
    }
    ))
}
...
function loadTransId(err, response) {
    ...
    if (response.success) {
        ...
        $('#transId-result').html(`<div class="alert alert-${data.status == 'win' ? 'success' : 'danger'}"><b> ${data.result == 'unknown' ? 'Không xác định' : (data.result == 'win' ? 'Chiến thắng' : 'Thua cuộc')} </b><br><b> Mã giao dịch: #${data.transId} </b> <br><b> Tiền cược: ${Intl.NumberFormat('en-US').format(data.amount)} </b> <br><b> Tiền nhận: ${Intl.NumberFormat('en-US').format(data.bonus)} </b> <br><b> Nội dung: ${data.comment} </b> <br><b> Thời gian: ${data.time} </b><hr>${refund}</div>`);
    }
}
